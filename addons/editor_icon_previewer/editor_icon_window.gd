...
@onready var deselect_button = icon_info.get_node("Deselect")
...
func _icon_gui_input(event, icon):
	if event is InputEventMouseButton and event.pressed:
		...
		elif event.button_index == MOUSE_BUTTON_RIGHT or event.button_index == MOUSE_BUTTON_MIDDLE: # Copy icon's name with embedded code into the clipboard
			DisplayServer.clipboard_set(SNIPPET_TEMPLATE % [icon.tooltip_text])
		icon_copied_label.show()
		selected_icon = icon
		deselect_button.visible = true
	elif event is InputEventMouseMotion:
		if icon != selected_icon: await get_tree().create_timer(0.5).timeout; icon_copied_label.hide()
...
func _update_icons():
	var number = 0
	for idx in range(previews_container.get_child_count()):
...
func _on_container_mouse_exited():
	if selected_icon == null:
		...
		icon_info_label.text = selected_icon.tooltip_text
		icon_preview.texture = selected_icon.texture
		icon_size_label.text = ICON_SIZE_MSG + str(selected_icon.texture.get_size())
...
func _on_button_pressed():
	...
	var path: String = $body/icons/info/icon/Export/LineEdit.text
	if !path.contains("://"): path = "res://" + path
	print(DirAccess.dir_exists_absolute(path))
	var error = img.save_png("%s/%s.png" % [path, $body/icons/info/icon/label.text])
	if error == OK: print("Saved %s to: %s/%s.png" % [$body/icons/info/icon/label.text, path, $body/icons/info/icon/label.text]); EditorInterface.get_resource_filesystem().scan()
	elif error == FAILED: print("Does the folder %s/ exist?" % path)
