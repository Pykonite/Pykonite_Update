...
@onready var property_input


func _enter_tree():
	property_input = OptionButton.new()
	add_control_to_container(CONTAINER_TOOLBAR, property_input)
	assert(Engine.get_version_info().major >= 3)

	icon_window = preload('editor_icon_window.tscn').instantiate()
	EditorInterface.get_base_control().add_child(icon_window)
...
func _populate_icons():
	...
	if godot_theme == null: push_error("Theme not found!"); return
...
#		node_input.text = selected_node_path
		...
		for p in properties:
			if p["type"] == "Texture": property_input.add_item(p.name)
